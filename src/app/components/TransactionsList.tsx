...
const TransactionList: React.FC<Props> = () => {
    ...
    useEffect(() => {
        ...
        const currentDate = `${new Date().getDate()}/${new Date().getMonth() + 1}/${new Date().getFullYear() + 543}`; //วันที่ปัจจุบัน
        ...
        if (userData) {
            ...
            const currentMonthThai = thaiMonthNames[new Date().getMonth()];
            const currentMonthKey = `${currentYear}/${currentMonthThai}`;
        }
    }, [currentDate]);
    
    // ฟังก์ชันสำหรับลบธุรกรรมตาม transactionId ที่รับเข้ามา
    const handleDelete = (transactionId: string) => {
        // กรองรายการที่ต้องการลบออกจาก currentTransactions
        const updatedTransactions = currentTransactions.filter((transaction: Transaction) => transaction.id !== transactionId);
        // ทำการเรียงลำดับรายการใหม่หลังจากการกรองแล้ว
        const sortedTransactions = updatedTransactions.sort((a: Transaction, b: Transaction) => {
            const dateA = new Date(a.date).getTime();
            const dateB = new Date(b.date).getTime();
            return dateB - dateA;
        }).reverse();
        // ทำการอัปเดต state ของ currentTransactions ด้วยรายการที่เรียงลำดับแล้ว
        setCurrentTransactions(sortedTransactions);
        // ทำการอัปเดตข้อมูลใน local storage ด้วยรายการที่เรียงลำดับแล้ว
        const userData = localStorage.getItem('user');
        if (userData) {
            const parsedUserData = JSON.parse(userData);
            const buddhistYear = new Date().getFullYear() + 543;
                ...
                transactions: {
                    ...
                    [currentYear]: {
                        ...
                        [new Date().getMonth() + 1]: {
                            ...
                            [currentDate]: {
                                data: sortedTransactions, // ใช้รายการที่เรียงลำดับแล้ว
                                totalToday: sortedTransactions.reduce((total: number, transaction: Transaction) => {
    ...
    return (
        <section className={TranSactions.section}>
            <header className={TranSactions.section__header}>
                ...
                <Link href="/reports">
                    <button className={TranSactions.section__header__button}>
