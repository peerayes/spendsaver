...
interface TransactionsData { // key-value string,any
    [formattedDate: string]: Transaction;
}
interface UserData { //เก็บตาม properties
    userName: string;
...
const MyReports = () => {
    const [userData, setUserData] = useState<UserData>({userName: '', transactions: {}, formattedAmount: 'xx.xx', walletTotal: 0});
    ...
    const [monthsWithTransactions, setMonthsWithTransactions] = useState<string[]>([]);
    ...
    const [walletsMonthTotal, setWalletsMonthTotal] = useState<{ [key: string]: number }>({});
    const [transactionSumLast7Days, setTransactionSumLast7Days]= useState<number>(0);
    const [totalWalletSumLast7Days, setTotalWalletSumLast7Days] = useState<number>(0);
    const [selectedMonthAverage, setSelectedMonthAverage] = useState<number | null>(null);
    ...
    const generateDoughnutData = useCallback((option: string, transactionSumLast7Days: number, totalWalletSumLast7Days: number) => {
        let data: number[] = [];
        if (option === "7 วันล่าสุด") {
            ...
            if (walletsMonthTotal) {
                const monthsOfYear = Object.keys(transactionsMonthTotal).filter(key => key.includes(year));
                const totalExpenseOfYear = monthsOfYear.reduce((acc, curr) => acc + transactionsMonthTotal[curr], 0);
                const totalWalletOfYear = monthsOfYear.reduce((acc, curr) => acc + walletsMonthTotal[curr], 0);
                data = [totalExpenseOfYear, totalWalletOfYear];
            }
        } else {
            const monthDataExpens = transactionsMonthTotal[`${currentYear}/${option}`];
    ...
    useEffect(() => {
        ...
        if (userData) {
            ...
            if (parsedUserData && parsedUserData.transactions) {
                ...
                for (const year in parsedUserData?.transactions) {
                    for (const month in parsedUserData.transactions[year]) {
                        for (const date in parsedUserData.transactions[year][month]) {
                            ...
                            last7DaysDataObject[currentMonthKey].push(`${day} = ${transactionEntry.totalToday}`);
                            transactionsMonthTotalArray[currentMonthKey].push(`${day} = ${transactionEntry.totalToday}`);
                        }
                    }
                }
                const last7DaysDataArray = Object.keys(last7DaysDataObject).reduce((acc: string[][], month) => {
                    const transactions = last7DaysDataObject[month];
                    if (transactions.length > 7) {
                        const slicedTransactions = transactions.slice(-7);
                        const formattedTransactions = slicedTransactions.map(transaction => {
                ...
                const transactionSumLast7Days = last7DaysDataArray.reduce((sum, dayData) => {
            ...
            if (parsedUserData && parsedUserData.wallet) {
                ...
                for (const year in parsedUserData?.wallet) {
                    for (const month in parsedUserData.wallet[year]) {
                        for (const date in parsedUserData.wallet[year][month]) {
                            ...
                            last7DaysWalletsObj[currentMonthKey].push(`${day} = ${walletEntry.totalWallet}`);
                        }
                    }
                }
                const walletLast7DaysDataArray = Object.keys(last7DaysWalletsObj).reduce((acc: string[][], month) => {
                ...
                const totalWalletSumLast7Days = walletLast7DaysDataArray.reduce((sum, dayData) => {
    ...
    useEffect(() => {
        ...
        const monthsWithTransactions = Object.keys(transactionsMonthTotalArray).map((monthKey) => {
    ...
    const generateGraphData = useCallback((option: string) => {
        let labels: string[] = [];
        let datasetData: number[] = [];
        if (option === '7 วันล่าสุด') {
            labels = last7DaysData.map(transaction => transaction[0].split(', ')[0]);
            datasetData = last7DaysData.map(transaction => parseInt(transaction[0].split(', ')[1]));
        } else if (option.startsWith('ปี')) {
            const year = option.split(' ')[1];
            if (transactionsMonthTotal) {
                labels = Object.keys(transactionsMonthTotal).map(month => month.split('/')[1]);
                datasetData = Object.values(transactionsMonthTotal);
            }
        } else {
            const monthData = transactionsMonthTotalArray[`${currentYear}/${option}`];
            if (monthData) {
                labels = monthData.map(entry => entry.split(' = ')[0]);
                datasetData = monthData.map(entry => parseInt(entry.split(' = ')[1]));
            }
        }
        const data = {
            labels: labels,
        ...
        return data;
    }, [currentYear, last7DaysData, transactionsMonthTotal, transactionsMonthTotalArray]);
    ...
    const calculateYearlyAverage = (transactionsMonthTotalArray: { [key: string]: string[] }, transactionsMonthTotal: { [key: string]: number }) => {
        let totalExpense = 0;
        let totalDays = 0;
    ...
    useEffect(() => {
        const calculatedAverage = calculateYearlyAverage(transactionsMonthTotalArray, transactionsMonthTotal);
        setYearlyAverage(calculatedAverage);
    }, [transactionsMonthTotalArray, transactionsMonthTotal]);

    const calculateMonthlyAverages = (transactionsMonthTotalArray: { [key: string]: string[] }) => {
        ...
        for (const monthKey in transactionsMonthTotalArray) {
            ...
            const monthlyAverage = Math.round(totalExpense / numberOfDays * 100) / 100;
    ...
    const renderMonthButtons = () => {
        return monthsWithTransactions.map((monthName) => (
            <li key={monthName} className={currentMonth === monthName ? "--active" : ""}>
                <button onClick={() => { handleOptionClick(monthName); console.log(`Clicked: ${monthName}`); }}>- {monthName}</button>
            </li>
        ));
    };
    ...
    useEffect(() => {
        generateDoughnutData(selectedOption, transactionSumLast7Days, totalWalletSumLast7Days);
    }, [selectedOption, transactionSumLast7Days, totalWalletSumLast7Days, generateDoughnutData]);
    ...
    useEffect(() => {
        ...
        const newTotalWalletSumLast7Days = last7DaysWalletData.reduce((sum, dayData) => {
            const totalwallet = parseInt(dayData[0]);
            return sum + totalwallet;
        }, 0);

        generateDoughnutData(selectedOption, newTransactionSumLast7Days, newTotalWalletSumLast7Days);
    }, [last7DaysData, last7DaysWalletData, selectedOption, generateDoughnutData]);
    ...
    const handleOptionClick = (option: string) => {
        ...
        if (option !== "7 วันล่าสุด") {
            ...
            generateDoughnutData(option, transactionSumLast7Days, totalWalletSumLast7Days);
        } 
        toggleDropdown();
    };
            ...
            <div className={ReportStyle.report__container}>
                ...
                <div className={ReportStyle.report__container__filter}>
                    <div className={ReportStyle.report__container__filter__dropdown}>
                        <button type='button' className={ReportStyle.report__container__filter__toggle} onClick={toggleDropdown}>
                        ...
                        {isOpen && (
                            <ul className={ReportStyle.report__container__filter__list}>
                                <li>
                                    <button type='button' onClick={() => { handleOptionClick('7 วันล่าสุด'); logButtonClick('7 วันล่าสุด'); }}>7 วันล่าสุด</button>
                                </li>
                                <ol>
                                    <li>รายเดือน</li>
                                    {renderMonthButtons()}
                                </ol>
                                <li><button type='button' onClick={() => { handleOptionClick('ปี 2567'); logButtonClick('ปี 2567'); }}>ปี 2567</button></li>
                            </ul>
                        )}
                    </div>
                </div>
            ...
            <div className={ReportStyle.report__summary}>
                ...
                <div className={ReportStyle.report__summary__right}>
                    <h3 className={ReportStyle.report__summary__right_title}>
                        <strong>ค่าเฉลี่ย</strong><small>วัน{format(new Date(), 'EEEE', { locale: th })},{currentDate}</small>
                    </h3>
                    <p className={ReportStyle.report__summary__right_desc}>วันนี้: <strong>{totalToday}฿</strong></p>
                    <p className={ReportStyle.report__summary__right_desc}>สัปดาห์: <strong>{Math.round(transactionSumLast7Days / 7)}฿/วัน</strong></p>
                    <p className={selectedOption !== "7 วันล่าสุด" && selectedOption !== "ปี 2567" ? ReportStyle.report__summary__right_desc : "hidden"}>
                        {`${selectedOption}`} <strong>{selectedMonthAverage}฿/วัน</strong>
                    </p>

                    <p className={ReportStyle.report__summary__right_desc}>ปี: <strong>{yearlyAverage}฿/วัน</strong></p>
                </div>
            </div>
        </div>
    );
