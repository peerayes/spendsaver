...
const WalletPage: React.FC = () => {
    ...
    if (userData) {
      ...
      if (parsedUserData && parsedUserData.wallet) {
        ...
        for (const year in parsedUserData.wallet) {
          for (const month in parsedUserData.wallet[year]) {
            for (const date in parsedUserData.wallet[year][month]) {
              const walletForCurrentDate =
                parsedUserData.wallet[year][month][date].data;
              if (walletForCurrentDate && Array.isArray(walletForCurrentDate)) {
                walletForCurrentDate.forEach((item: WalletDataItem) => {
                  const monthYear = `${year}/${
                    thaiMonthNames[parseInt(month, 10) - 1]
                  }`;
        ...
        for (const monthYear in sortedWalletData) {
          sortedWalletData[monthYear]
            .sort((a: WalletDataItem, b: WalletDataItem) => {
  ...
  const handleDelete = (id: string) => {
    setWalletData((prevData) => {
      ...
      for (const monthYear in updatedWalletData) {
        updatedWalletData[monthYear] = updatedWalletData[monthYear].filter(
          (item) => item.id !== id
        );
      }
      return updatedWalletData;
    });
    const newTotal = Object.values(walletData).reduce(
      (acc, monthData) =>
        acc +
        monthData.reduce((monthTotal, item) => monthTotal + item.amount, 0),
      0
    );
    ...
    const updatedWallet: {
      [year: string]: {
        [month: string]: {
          [day: string]: { data: Transaction[]; totalWallet: number };
        };
      };
    } = { ...user.wallet };
    for (const year in updatedWallet) {
      for (const month in updatedWallet[year]) {
        for (const date in updatedWallet[year][month]) {
          updatedWallet[year][month][date].data = updatedWallet[year][month][
            date
          ].data.filter((item: Transaction) => item.id !== id);
          updatedWallet[year][month][date].totalWallet = updatedWallet[year][
            month
          ][date].data.reduce(
            (total: number, item: Transaction) => total + item.amount,
            0
          );
        }
      }
    ...
    localStorage.setItem(
      "user",
      JSON.stringify({ ...user, wallet: updatedWallet })
    );
  };
  ...
  const handleAddMoney = () => {
    ...
    if (!isNaN(parsedMoney)) {
      ...
      const totalAmount = Object.values(updatedWallet).reduce(
        (acc, year) =>
          acc +
          Object.values(year).reduce(
            (accMonth, month) =>
              ...
              Object.values(month).reduce(
                (accDay, day) => accDay + day.totalWallet,
                0
              ),
            0
          ),
  ...
  const handleOutsideClick = (event: MouseEvent) => {
    ...
    const walletAmountElement = document.querySelector(
      `.${Wallet.wallet__bag__money}`
    );
  ...
  return (
    <div className={Wallet.wallet}>
      <header className={Wallet.wallet__header}>
        <h2>Wallet</h2>
        <button
          className={Wallet.wallet__header__span}
        ...
        <div className={Wallet.wallet__profile}>
          <div className={Wallet.wallet__bag}>
            ...
            <div className={Wallet.wallet__bag__amount}>
              ...
              />
              <button
                className={`${buttonStyle.btn} ${buttonStyle.btn__addMoney}`}
        ...
        <ul className={Wallet.transaction__listitem}>
          {Object.keys(walletData)
            ...
            .map((monthYear) => {
              ...
              const sortedTransactions = walletData[monthYear]
                .slice()
                .sort((a, b) => {
              ...
              return (
                <li
                  key={monthYear}
                  className={Wallet.transaction__listitem__items}
                >
                  <h3>
                    <span>
                      <svg
                      ...
                      {thaiMonths[parseInt(month) - 1]} {year}
                    </span>
                  </h3>
                  {sortedTransactions.map(
                    (dataItem: WalletDataItem, index: number) => (
                      <ol
                        key={index}
                        className={Wallet.transaction__listitem__daily}
                      >
                        <li className={Wallet.daily}>
                          <div className={Wallet.daily__date}>
                            {dataItem.date}
                          </div>
                          <div className={Wallet.daily__text}>
                            {dataItem.text}
                          </div>
                          <div className={Wallet.daily__amount}>
                            {dataItem.amount} บาท
                          </div>
                          <div className={Wallet.daily__btn}>
                            <button
                              className={Wallet.transaction__delete}
